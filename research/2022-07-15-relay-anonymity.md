...
For each definition, we summarize Wakus current guarantees regarding the respective property.
...
The concepts of security, privacy, and anonymity are linked and have a quite a bit of overlap.
...
Because each topic is associated with a content topic, and each receiver is interested in messages with specific content topics,
...
In the future, Waku might distribute messages over several libp2p pubsub topics.
This would still yield $k$-anonymity, assuming $k$ content topics share the same pubsub topic.
Waku is not directly concerned with 1:1 communication, so for this post, 1:1 communication is out of scope.
A 1:1 communication protocol might be added to Waku in the future.
It would maintain receiver anonymity leveraging [partitioned topics](https://specs.status.im/spec/10#partitioned-topic).
...
This could be a rouge access point (with routing capability).
...
We plan to work mix-net integration, which will allow Waku to be anonymity-preserving even within the strongest attacker models.
For applications that have specific requirements, e.g. lower latency, that cannot be provided on top of mix-nets,
we plan to integrate solutions that meet these requirements at the cost lowering the anonymity level.
An example might be messages over a Tor-like onion routing layer, which would offer low-latency transport.
We refer to these as pluggable anonymity-preserving protocols; they will form a sub-set of the Waku v2 protocol set.
As an intermediate step, we might directly employ Tor for onion-routing, and [Nym](https://nymtech.net/) as a mix-net layer.
[Dandilion++](https://arxiv.org/abs/1805.11060) is also on list of techniques we are investigating and plan to offer as a lightweight pluggable anonymity protocol.

In future research log posts, we plan to cover further Waku v2 protocols and identify anonymity problems that will be added to our roadmap.

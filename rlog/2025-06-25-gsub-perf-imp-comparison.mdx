...
This post provides a concise overview and performance evaluation of ongoing efforts aimed at improving the GossipSub protocol's performance.
{/* truncate */}

## Overview

For every subscribed topic, GossipSub nodes maintain a full-message mesh comprising $D$ peers 
and a metadata-only (gossip) mesh comprising additional $K$ peers. 
...
replying to IWANT requests is optional, 
which encourages any peer to make redundant IWANT requests for the same message. 
While this arrangement works well for small messages, it can be inefficient for bigger ones. 
This inefficiency arises from an increased number of duplicates 
and a higher volume of IWANT requests due to longer transmission times for large messages. 
...
1. A longer contention interval ($\tau_D$) raises the likelihood of simultaneous redundant transmissions, 
thereby increasing the number of duplicates. 
Reducing $D$ inadvertently increases $h$.
...
we get $\frac {N \times D}{2}$ transmissions. 
...
Out of these, $N(D-1)+1-(N-1)$ messages are duplicates, 
which simplifies the upper bound on average duplicates received by each peer to $\bar{d}_{max} \approx D-2$. 
This rise in average duplicates indicates the impact of increasing message size on the number of duplicates. 
It is worth mentioning that the impact of IWANT/IDONTWANT messages is not considered in $\bar{d}$ computations above.
...
the peers forwarding the message should gradually reduce the number of mesh members that directly receive (push) 
the message and send immediate IHAVE announcements to the remaining peers in the mesh. 
...
However, this approach may compromise the publisher's anonymity and may reveal information about its mesh members. 
...
The GossipSub v1.4 proposal defines the sending and processing of PREAMBLE/IMRECEIVING messages as optional. 
While the proposal introduces an optional safety strategy, 
it leaves room for implementations to use custom safety strategies.
...
Additionally, a production-grade implementation is currently under development 
...
that combines both push and pull strategies through two new control messages: 
IANNOUNCE and INEED. 

The balance between push and pull approach is determined by the $D_{announce}$ parameter. 
...
IANNOUNCE acts as an IHAVE announcement, signaling the availability of a message. 
On receiving an IANNOUNCE for an unseen message, 
a peer can request it using an INEED message. 
It is important to note that a peer can send only one unfinished INEED request per message. 
If the message transfer does not complete in the specified message transfer interval, 
the receiver can issue a new INEED request. 


#### Key Considerations and [PoC Implementation](https://github.com/vacp2p/nim-libp2p/tree/research_gs_v2_0)

The $D_announce$ parameter governs the balance between push and pull operations. 
...
The authors propose a timeout strategy to address these issues.
...
To support these evaluations, we extended the nim-libp2p implementation to incorporate the proposed protocols 
[[16](https://github.com/vacp2p/nim-libp2p/tree/research_gs_v1_4),
...
Bandwidth refers to the total traffic volume, 
encompassing both control traffic and actual data transmissions. 
...
[17] [GossipSub v1.4: Production Implementation in nim-libp2p](https://github.com/vacp2p/nim-libp2p/pull/1448)

...
A hash function $\mathsf{Hash}$ is a special function that takes in an arbitrary binary string and outputs a bit string of a constant length.
Specifically,
$\mathsf{Hash} : \{0,1\}^* \rightarrow \{0,1\}^n$.

Informally, a cryptographic hash function $\mathsf{Hash}$ satisfies the following properties:
- **Succinct**: The hash $\mathsf{Hash}({\bf{b}})$ can be efficiently computed for any binary string ${\bf{b}}$.

...
the number of peers receiving the message increases by $2^r\ \forall\ r \in \{0, D-1\}$ and by $\sum_{k=r-D}^{r-1} \lambda_k\ \forall\ r \geq D$. 
Here, $r$ represents message transmission round $r = \frac{\tau_D}{D} \text{ only if } D \mid \tau_D$, and $\lambda_k$ represents the number of peers that received the message in round $k$.

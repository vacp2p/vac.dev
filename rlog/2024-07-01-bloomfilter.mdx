...
In this blog, we examine data Bloom filters and Cuckoo filters.
Both of these filters are data structure that can be used for membership proofs.
...
**Bloom filters** and *Cuckoo filters** are data structures can be used to accumulate data with a fixed amount of space.
...
The algorithms associated to Bloom filters and Cuckoo filters, that we will discuss shortly, are deterministic.
The possibility of false-positives arises from the query algorithm.


## Bloom Filters
**Bloom filter** is a data structure that can be used to accumulate an arbitrary amount of data with a fixed amount of space.
...
Specifically, we obtain the bits we wish to flip to 1's by computing $h(X || i)$ for each $i \in \{0,\dots, k-1\}$ and $X$ as we will define next.
For Sliding-Window Bloom filters, $X$ is more than just the element we wish to append to the filter.
Instead, $X$ consists of the message $x$ and a timestamp $t$. 
...
A fingerprints can be thought of as the hash value for an entry in the digest.
...
Now, we describe the algorithms associated to the Cuckoo filter $C$ with hash function $hash(X)$ and fingerprint function $fingerprint(X)$.
...
Instead, we insert this fingerprint to its alternate location.
...
As such, append does not run in constant-time.

## Bloom Filters vs Cuckoo Filters
The design of Bloom filters is focused on space-efficiency and a quick query time.
Even though Cuckoo filters occupy constant space,
Cuckoo filters require significantly more space for $n$ items than Bloom filters.
The worst case append in Cuckoo filter is slower than append in Bloom filter.
...
Further, the probability of false-positives for Cuckoo filters is lower than the probability of false-positives in Bloom filters.

...
which include Poseidon [[1]](#references) — 
a well-known hash function for zk-SNARK proving systems.
...
The article aims to introduce MDSECheck method — 
...
*iff* for any distinct $n$-dimensional column vectors $v_1$ and $v_2$
...
*iff* all its square submatrices are nonsingular.
...
for which all elements in the concatenation of $x$ and $y$ are distinct.
The elements of the Cauchy matrix are described by the formula 
$M_{i, j} = 1 \: / \: (x_i - y_j)$.
...
*iff* there is a proper subspace of differences of internal state vectors,
...
*iff* $w$ divides $u$ and $w < u$ [[7]](#references), 
...
as the components of affine permutation boxes of P-SPNs.

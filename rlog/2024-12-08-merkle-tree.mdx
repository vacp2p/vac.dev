...
The Merkle tree is able to bind a digest with one of the properties of hash functions (see our previous Vac-101 [[2](https://vac.dev/rlog/vac101-fiat-shamir#hash-functions)] for information on hash functions).
A hash function is collision resistant; it is difficult for a malicious party to find two values share the same hash value.
...
This can be done with logarthmic storage and logartihmic computation time.
...
Suppose that the Prover provides data $\ell$ to the Verifier,
...
This pathing continues until the Verifier either successfully computes the root hash (in $n+1$ hashes) or fails to.
...
A SMT is treated as a Merkle tree in which each entry is placed in the leaf corresponding to their hash value, and the other entries have a $\mathsf{null}$ marker inserted in.
This means that we can prove membership in the way as described.
...
We can take advantage of the sparse nature of SMT to provide shorten proofs.
...
These values can be used to shorten the time to construct a SMT, and the length of proofs.

### Proof of nonmembership

In the first Vac-101 [[4](https://vac.dev/rlog/vac101-membership-with-bloom-filters-and-cuckoo-filters)], we examined Bloom and Cuckoo filters that could be used for proof of membership and nonmembership.
...
We note that the requirement that a SMT is sorted can be too strong of an assumption in centralized cases.
However, sorted is a necessary property of SMT for decentralized systems.
...
- 1. [Secrey, Authentication, and Public Key Systems](https://www.ralphmerkle.com/papers/Thesis1979.pdf) 
- 2. [Vac 101: Transforming an Interactive Protocol to a Noninteractive Argument](https://vac.dev/rlog/vac101-fiat-shamir#hash-functions)
- 3. [Vac 101: Membership with Bloom Filters and Cuckoo Filters](https://vac.dev/rlog/vac101-membership-with-bloom-filters-and-cuckoo-filters)
- 4. [Verkle Trees](https://math.mit.edu/research/highschool/primes/materials/2018/Kuszmaul.pdf)
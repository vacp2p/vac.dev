...
The "membership set" described above, is in the form of a sparse, indexed merkle tree.
This membership set, can be on-chain, off-chain or hybrid depending on the network's storage costs.
Waku makes use of a hybrid membership set, 
where insertions are tracked in the smart contract.
Each Waku node maintains a local copy of the tree, 
which is updated upon each insertion.

A User registers with a hash of a locally generated secret, 
which is then inserted into the tree at the next available index.
Upon which, Users can prove their knowledge of the pre-image of a leaf in the tree, 
also called a commitment.
They can do so by generating a [Merkle Inclusion Proof](https://ethereum.org/en/developers/tutorials/merkle-proofs-for-offline-data-integrity/).
...
After a User generates this zk-proof, 
they can transmit it to other users, 
who can verify the proof.
Since the message's hash is included within the proof generation, 
we also guarantee integrity of the message by this mechanism.

When a User does behave maliciously, 
they generate multiple proofs per epoch.
When multiple proofs are generated per epoch, 
this exposes the malicious User's secret, further described in [Malicious User secret interpolation mechanism](#malicious-user-secret-interpolation-mechanism)
...
6. Messages per epoch: `1`
7. Epoch duration: `10 seconds`
...
34 megabytes of storage is trivial to obtain on most end-user hardware, with the exception of IoT/microcontrollers.
Optimizations are planned where the user does not need to store the entire tree to be able to generate proofs.

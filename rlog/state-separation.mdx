...
NSSA gives users control over their privacy settings by introducing shielded (which create a layer of privacy for the outputs and only the necessary details are shared) 
and deshielded (which reveal private details making them publicly visible) execution types, in addition to the traditional public and private modes. This flexibility allows 
...
into two states: public and private. The public state uses an account-based model, while the private state employs a UTXO-based model. Private executions within NSSA utilize 
...
### Why NSSA is different than other execution hybrid platforms
...
diverse privacy needs of users. These platforms blend public and private states, but the control over privacy remains with the application developers. 
...
Nescience State Separation architecture addresses these limitations by introducing a groundbreaking concept: **selective privacy**. Unlike traditional platforms where privacy 
...
NSSA is distinct from traditional zero-knowledge (ZK) roll-ups in several key ways. One of the unique features of NSS is its **public execution type**, which does not 
require Zero-Knowledge Proofs or a zkVM. This provides a significant advantage in terms of scalability and efficiency, as users can choose public executions for 
transactions that do not require enhanced privacy, avoiding the overhead associated with ZKP generation and verification.

Moreover, NSSA introduces two additional execution types—**shielded and deshielded**—which further distinguish it from traditional privacy-preserving roll-ups. 
...
By combining selective privacy with a flexible execution model, Nescience State Separation architecture offers a more robust and adaptable framework for decentralized applications, 
...
### How Nescience State Separation Architecture (NSSA) Can Be Used

The **Nescience State Separation Architecture offers a flexible, privacy-preserving add-on that can be applied to existing decentralized applications (Dapps). 
...
the deployment of millions of rollups in the near future. A key question arises: how many of these rollups will incorporate privacy? For Dapp developers who want to offer flexible, 
...
With NSSA, the privacy settings are flexible, meaning users can tailor their level of privacy according to their individual needs, all while the Dapp operates on its current infrastructure. 
...
**Example Application: Selective Privacy for Dapps**

Let’s take an example of a Dapp operating on a fully transparent blockchain. Users of this Dapp currently have no control over their transaction privacy; everything is public. 
If the Dapp developers want to offer a privacy option using NSSA, they can deploy a version of the Dapp with the **Nescience state-separation model** integrated into it. 
This version would allow users to choose selective privacy, deciding whether they want their transactions to be public, private, shielded, or deshielded. 

For example, users engaging in sensitive financial transactions might opt for full privacy, while users making routine, low-stakes transactions might prefer to keep them public 
for simplicity and efficiency. NSSA enables this flexibility, even though the Dapp itself originally did not support privacy features.
...
By deploying the Nescience state-separation architecture as an add-on, Dapps can give users the ability to choose their own privacy settings, whether they are operating on 
...
These topics will provide a comprehensive understanding of how NSSA enables flexible and secure interactions within decentralized applications.

## 1. Architecture's components
---

The Nescience State Separation Architecture (NSSA) introduces an advanced prototype execution framework designed to enhance privacy and security in blockchain applications. 
This framework integrates several essential components: the public state, private state, Zero-Knowledge Virtual Machine (zkVM), various execution types, Nescience users, and smart contracts.

### a) Public State
---

The public state in the Nescience State Separation Architecture (NSSA) is a fundamental component designed to hold all publicly accessible information within 
...
making it suitable for applications that require public scrutiny and auditability, such as public record updates and financial transactions.
...
### b) Private State
---
The private state in the Nescience State Separation Architecture (NSSA) is a sophisticated system designed to maintain user privacy while ensuring transaction integrity. 
...
which uses an account-based model. Instead, the private state employs a UTXO (Unspent Transaction Output)-based model. In this model, each transaction output is a discrete 
...
A key aspect of maintaining privacy within the private state is the use of Zero-Knowledge Proofs (ZKPs). ZKPs allow transactions to be validated without revealing any 
...
The private state also employs a dual-storage approach to balance privacy and transparency. UTXOs are stored in plaintext within Sparse Merkle Trees (SMTs) in the private state, 
...
which are then appended to the user’s private list. The zkVM (Zero-Knowledge Virtual Machine) generates a zero-knowledge proof to validate the transaction without revealing 
...
By utilizing a UTXO-based model, individual Merkle trees, Zero-Knowledge Proofs, and a dual-storage system, NSSA achieves a balance between confidentiality and verifiability, 
making it a robust solution for managing private blockchain transactions.

### c) zkVM (Zero-Knowledge Virtual Machine)
---

The Zero-Knowledge Virtual Machine (zkVM) is a pivotal component in the Nescience State Separation Architecture (NSSA), designed to uphold the highest standards 
of privacy and security in blockchain transactions. Its primary function is to generate and aggregate zero-knowledge proofs (ZKPs), enabling users to validate the 
...
Zero-knowledge proofs are sophisticated cryptographic protocols that allow one party, the prover, to convince another party, the verifier, that a certain statement is true, 
...
exposing transaction specifics, such as amounts or involved parties. This process is essential for transactions within the private state, where maintaining confidentiality is paramount.
...
aggregate zero-knowledge proofs not only preserves the confidentiality of user data but also enhances the overall efficiency and scalability of the blockchain network. 
...
The Nescience State Separation Architecture (NSSA) incorporates multiple execution types to cater to varying levels of privacy and security requirements. 
...
by all participants in the network. Public executions do not require zero-knowledge proofs (ZKPs) since transparency is the primary goal. This execution type is ideal 
...
In a private execution, the user initiates a transaction involving private data. The transaction spends existing UTXOs and creates new ones, all represented as 
leaves in the Merkle tree. The zkVM generates a zero-knowledge proof to validate the transaction without revealing private data. This proof is submitted to the sequencer, 
...
and the private state is prepared to receive new data. The zkVM generates a zero-knowledge proof to hide the receiver’s identity. This proof is submitted to the sequencer, which verifies 
...
and the transaction details are prepared. The zkVM generates a zero-knowledge proof to hide the sender’s identity. This proof is then submitted to the sequencer, 
...
| Private    | Private State| Private State| Yes         | Confidential transactions needing privacy.                              | Suitable for transactions that require confidentiality. Ensures that transaction details remain private through the use of zero-knowledge proofs.                                                                                                    |
...
Security is ensured through the use of cryptographic keys, which authenticate and verify transactions. Zero-Knowledge Proofs (ZKPs) maintain privacy 
...
The process begins when a user initiates a transaction by invoking a smart contract (SC). This invocation involves selecting at least one of 
...
In what follows, the execution process of the four different execution types within the Nescience architecture:
...
* Decryption by the Receiver: After the broadcast, the receiver attempts to decrypt the broadcasted UTXOs using their symmetric key, derived from the ephemeral public key. 
...
* Stage 2 (UTXO Minting and Privacy): New UTXOs are minted in the private state, encrypted, and broadcasted to the network. The transaction remains private from this point forward, 
...
private keys handle the operational aspects, while the spending key remains securely in the background. For example, when Alice decides to spend a UTXO in a 
...
The signing key (${private}_{key}.sig$) is the third and final component of the private key, used primarily for signing transactions. One possible solution is that
 Nescience employs Schnorr signatures, a cryptographic protocol known for its efficiency and security. I nthis case, the ${private}_{key}.sig$ component would generate 
 ...
 signatures are a important as they provide a secure and non-repudiable method of verifying that a transaction was initiated by the legitimate owner of the assets. 
...
broadcasted to the network, ${public}_{key}.sig$ enables any participant to verify that the transaction’s signature matches the user’s private key, without 
...
3. ${public}_{key}.sk(prf)$: Pseudorandom Function (PRF)
...
a transparent view of their interactions while keeping their assets secure. For example, if Alice is using a shielded execution to transfer assets, her viewing key enables her to 
...
o maintain the unlinkability and confidentiality of private transactions, the viewing key must be kept secure at all times.
...
This could include simple token transfers, public contract calls, or interactions with decentralized applications (dApps) that require public accountability, 
such as voting or governance systems.

#### b) Private Addresses
In contrast, private addresses are designed for confidentiality and are not visible on-chain. These addresses are used exclusively for private transactions and executions, 
...
Public addresses are visible on-chain and are used for open, public interactions that require accountability and auditability. In contrast, private addresses are 
invisible on-chain and are used for confidential transactions, enhancing privacy and security.
...
3. UTXO Trees (Private States): Each user or smart contract (SC) has its private state stored in UTXO trees. These trees are kept as plaintext on the client’s 
local system (off-chain), ensuring privacy as sensitive information remains confidential. The private state includes all UTXOs owned by the user or SC, and these 
...
so even if the data is intercepted, it cannot be deciphered without the appropriate decryption keys. One of the crucial privacy-preserving features is the support 
...
his makes it accessible to a wide range of users without requiring specialized equipment or significant computational resources.
...
Nescience is developing an **MPC (Multi-Party Computation)** based synchronization mechanism to balance privacy and fairness between public and private execution types. 
...
[4] Wuille, P. (2012). Bitcoin Improvement Proposal (BIP) 44. Retrieved from https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki

[5] Design Principles of Private Transactions in Aleo & Zcash. Retrieved from https://hackmd.io/@sin7y/rkxFXLkgs
...
[7] zcash tech specs. Retrieved from https://zips.z.cash/protocol/protocol.pdf
...
[10] Zero-knowledge Virtual Machine. Retrieved from https://medium.com/@abhilashkrish/zero-knowledge-virtual-machine-zkvm-95adc2082cfd

...
authors: seemenkina, seugu
published: true
slug: de-mls-with-waku
image: /img/de-mls/flow.PNG
categories: research
...
One scalable solution for SGM is Message Layer Security (MLS).
...
Multiple special nodes in the group serve as authorized identities to authenticate users
before they join or are removed from the group.

de-MLS provides two different user management configurations, both utilizing the Waku protocol for DS:

1. **Single Admin**:
    - A single authorized identity (admin) manages the group,
...
2. **Multi-Admin**:
    ...
    In each epoch, all modifications are managed exclusively by a single admin.

In multi-admin settings, de-MLS requires a consensus among admins
that have equal rights in the group since changes in an epoch in MLS are required
to be conducted by a single identity, that is the admin.
...
when a user wants to join a group in both admin and users side also their interactions.
...
The welcome topic is a topic created and monitored by the admin for a specific secure messaging group,
...
which the admin listens for and processes before granting access to the secure group.
...
Admin creates group announcement (GA) periodically to the welcome channel
that the users can find the who the admin is.
...
Then user can find the group name and also corresponding GA message from admin.
...
### 5. User creates the its key package

User creates the `keyPackage` and encrypt by public key of the Admin then send it to the admin.
Since the message is encrypted, stay secure though the welcome (permissionless) topic.

### 6. Admin receives the User's key package

Admin receives the user's `keyPackage` and decrypt it.
After decrypted, Admin also verifies the validity of the `keyPackage` by signature verification.
If the `keyPackage` is not valid, Admin just drop the message,
otherwise move the next step which is proposal creation.

### 7. Admin creates proposal

Proposals are special messages that may come from any participant including Admin.
In this case, Admin creates proposal corresponding user's `keyPackage`.
...
Ones admin creates the proposal send it to network for the voting.
If the result of the consensus is YES, lastly admin sends two different message.
...
After admin gets the YES from consensus, admin creates commit messages 
...
the admin message ensures that both existing and new users converge on the same group key in the next epoch.
...
- Firstly, the admin must receive the key packages from each member wishing to join the group.
...
This includes generating the admin key, creating messages with signatures and encryption,
...
- DoS attack to admin: Admin is known in welcome message from periodic group announcement message
so admin can be targeted for DoS attack.
...
- Dual-Consensus Multi-Admin Support: One consensus mechanism selects an admin from all users,
while a second governs group decisions among the elected admins

...
participants in the protocol in the correct order and manage the `keyPackage` of the users where the `keyPackage` is the objects that provide some public information about a user.
...
Waku integration is a crucial step in the construction of de-MLS, aiming to replace traditional client-server communication with decentralized messaging. The main challenge in this transition is transforming the centralized Delivery Service (DS) into a decentralized equivalent, which performs two essential functions:
...
Instead, our solution is to require users to actively provide their key packages upon request, allowing validation at the moment of use without persistent storage. This ensures both compliance with MLS's security model and alignment with decentralized system principles.

### Flow

The flow section explains the processes that when a user wants to join a group in both admin and users side also their interactions.  The flow of de-MLS is as follows:

![Figure 1](/img/de-mls/flow.png)

### 1. Admin joins the welcome topic

The welcome topic is a topic created and monitored by the admin for a specific secure messaging group, allowing any Waku node to subscribe permissionlessly. Being in the welcome topic does not imply group membership, it acts as a waiting room where users can send their key material, which the admin listens for and processes before granting access to the secure group.

### 2. Group initialization

Admin initalizes a group with parameters such as cipher suite and group ID.

### 3. Emitting Group Anouncement (GA) by admin

Admin creates group announcement (GA) periodically to the welcome channel that the users can find the who the admin is. This will be important for the next step.

### 4. User joins the welcome topic

As first, the users who wants to be part of the decentralized MLS should subscribe the welcome channel. Then user can find the group name and also corresponding GA message from admin. This GA message helps the user to create a valid keyPackages section 2 RFC9420 for the group.

### 5. User creates the its keyPackage

User creates the keyPackage and encrypt by public key of the Admin  then send it to the admin. Since the message is encrypted, stay secure though the welcome (permissionless) topic.

### 6. Admin receives the User's keyPackage

Admin receives the user's keyPackage and decrypt it. After decrypted, Admin also verifies the validity of the keyPackage by signature verification. If the keyPackage is not valid, Admin just drop the message, otherwise move the next step which is proposal creation.

### 7. Admin creates proposal

Proposals are special messages that may come from any participant including Admin. In this case, Admin creates proposal corresponding user's keyPackage. In regular MLS, proposals are automatically converted to the commit messages which has the ability the change the structure of the tree but in de-MLS, since it is decentralized, proposals should be voted before converted into the commitment.

### 8. Voting for proposal

Voting applies decentrazliation by protecting small groups can control. Therefore, proposals must be voted before committing. The consensus mechanism should be a lightweight consensus that cannot be bottleneck for treeKEM scalability. Basically, the consensus returns the binary result for given a proposal. If the result is NO, the proposal is dropped otherwise process.

Ones admin creates the proposal send it to network for the voting. ıf the result of the consensus is YES, lastly admin sends two different message.

### 9. Creating commit message

Commit messages are the messages that starts new epochs. They include key and tree material that existing members can generate the new state of tree.

After admin gets the YES from consensus, admin creates commit messages that inject new entropy for the existing group members.

### 10. Sending messages

After admin creates then sends two messages:
...
Although existing users had different group keys in the previous epoch and the new user had none, the admin message ensures that both existing and new users converge on the same group key in the next epoch.

### 11. Applying welcome message

User can generate the next epoch group key by using welcome message as well as existing users extract the same groupKey by using commit mesages.

With the commit message in step 10.2. helps the existing members generate the next groupKey Gk+1, which is different from Gk, and also welcome message 7.2. helps the newly joining user to generate Gk+1. Therefore, the new joining user cannot read the previous message since they encrypted by the Gk, it provides the forward secrecy (FS) and also if there would be dropped user who only knows the Gk, cannot read the further messages since they will encrypt with Gk+1, this provides post-comprimise security (PCS).
...
- Offline users: KeyPackages are provided by the users directly without any storing, this is required each user must be online for joining to a group.
    - We can consider to use [Waku sync nodes](https://docs.waku.org/guides/js-waku/store-retrieve-messages/) that are nodes has storing ability for a temporary storing of keyPackages.
- DoS attack to admin: Admin is known in welcome message from periodic group announcement message so admin can be targeted for DoS attack.
...
- Message loss or delay : Because of P2P and consensus settings, message can be lost or delayed.,
    - We can integrate reliability mechanisms to Waku such as [scalable data sync (SDS)](https://github.com/waku-org/nim-sds)
...
| Commit Message Handling | Relies on a server | Relies on a consensus and transparent admin |
...
- Dual-Consensus Multi-Admin Support: One consensus mechanism selects an admin from all users, while a second governs group decisions among the elected admins

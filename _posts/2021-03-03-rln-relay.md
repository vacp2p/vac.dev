...
- the lack of Shamir logic[^19] which is resolved and now is part of the RLN repository[^4]
- the concern regarding the potential multi-party computation for the trusted setup of zkSNARKs which got resolved[^20]
- The end-to-end integration was the final unknown that now we made it possible, have it implemented and  are going to present it in this post. New blockers are also sorted out during the e2e integration which we will discuss in the [Feasibility and Open Issues](#feasibility-and-open-issues) section.
...
<!-- We've come a long way since a year ago, blockers[^18] resolved, which include the proof time[^22], prover key size[^21], the lack of Shamir logic[^19], the zkSNARKs trusted setup[^20], and lastly the end-to-end integration which now we have implemented.--> 

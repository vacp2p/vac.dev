...
We've come a long way since a year ago, blockers[^18] resolved, which include the proof time[^22], prover key size[^21], the lack of Shamir logic[^19], the zkSNARKs trusted setup[^20], and lastly the end-to-end integration which now we have implemented. We learned lot and could identify further issues and unknowns some of which are blocking getting to production. The summary of the identified issues  are presented below.

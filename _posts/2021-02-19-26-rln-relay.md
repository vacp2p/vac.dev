...
We've come a long way since a year ago, blockers[^18] resolved, which include the proof time[^22], prover key size[^21], the lack of Shamir logic[^19], the zkSNARKs trusted setup MPC[^20], and lastly the end-to-end integration that now we have implemented it. We learned lot, and identified more issues and unknowns. Still some issues blocking getting to production which are discussed below.

...
categories: research
summary: Waku RLN-Relay utilizes the rate limiting nullifier method to enable a privacy-preserving spam-protection mechanism.
image: 
discuss: 
---
...
This post is going to give you an overview of how spam protection can be achieved in Waku Relay through rate-limiting nullifiers or RLN for short.

Let me give a little background about [Waku v2](https://github.com/vacp2p/specs/blob/master/specs/waku/v2/waku-v2.md). Waku is a privacy-preserving peer-to-peer (p2p) messaging protocol for resource restricted devices. Being p2p means that Waku relies on **No** central server. Instead, peers collaboratively deliver messages in the network. Waku follows [GossipSub](https://docs.libp2p.io/concepts/publish-subscribe/) as the underlying routing protocol (as the writeup of this post). At a high level, GossipSub is based on publisher-subscriber architecture. That is, *peers congregate around topics they are interested in. Peers can send messages to topics. Each message gets delivered to all peers subscribed to the topic*. In GossipSub, a peer has a constant number of direct connections / neighbors. A publishing peer forwards its message to a subset of its neighbors. The neighbors proceed similarly till the message gets propagated in the network of the subscribed peers.

## What do we mean by spam?

What does spamming stand for? In centralized messaging systems, a spammer usually indicates an entity that uses the messaging system to send an unsolicited message (spam) to large numbers of recipients. However, in the case of Waku, spam messages not only affect the recipients but also all the other peers involved in the routing process as they have to spend their computational power/bandwidth/storage capacity on processing spam messages. As such, we define a spammer as an entity who uses the messaging system to publish a large number of messages in a short amount of time. The messages issued in this way are called spam. In this definition, we disregard the intention of the spammer as well as  the content and the number of recipients.

## Related Solutions

Has the spamming issue addressed before? Of course yes! Here is an overview of the spam protection techniques with their trade-offs and use-cases. In this overview, we distinguish between protection techniques that are targeted for centralized messaging systems and those for p2p architectures.

### Centralized Messaging Systems
In traditional centralized messaging systems, spam usually signifies unsolicited messages sent in bulk or messages with malicious content like malware. Protection mechanisms include authentication through some piece of personal information e.g., phone number, checksum-based filtering (to protect against messages sent in bulk), challenge-response systems, and content filtering (on the server or via a proxy application). These methods exploit the fact that the messaging system is centralized and a global view of the users' activities is available based on which spamming patterns can be extracted and defeated accordingly.  Moreover, users are associated with an identifier (username) which enables the server to profile each user (to detect suspicious behavior like spamming) which is against the user's anonymity and privacy. 
<!-- I'd elaborate a bit on how something like phone numbers raise cost for registration, and then it can be cut off from system. This makes spam, as defined within the system and not through e.g. DDoSing Signal's servers, very costly.-->
...
#### Proof of work
The idea behind the proof of work i.e., [POW](https://www.investopedia.com/terms/p/proof-work.asp) is to make messaging a computationally costly operation hence lowering the messaging rate of **all** the peers including the spammers. In specific, the message publisher has to solve a puzzle and the puzzle is to find a nonce such that the hash of the message concatenated with the nonce has at least z leading zeros. z is known as the difficulty of the puzzle. Since the hash function is one-way, peers have to brute-force to find a nonce. Hashing is a computationally-heavy operation so is the brute-force. POW is also used as the underlying mining algorithm in Ethereum and Bitcoin blockchain. There, the goals is to contain the mining speed and allow the decentralized  network to come to consensus, or agree on things like account balances and the order of transactions. 
...
#### Peer Scoring
The peer scoring method that is utilized by [libp2p](https://github.com/libp2p/specs/blob/master/pubsub/gossipsub/gossipsub-v1.1.md#peer-scoring) is to limit the number of messages issued by a peer in connection to another peer. That is each peer monitors all the peers to which it is directly connected and adjusts their messaging quota i.e., to route or not route their messages depending on their past activities. For example, if a peer detects its neighbor is sending more than x messages per month, can drop its quota to z.x where z is less than one.  The shortcoming of this solution is that scoring is based on peers' local observations and the concept of the score is defined in relation to one single peer. This leaves room for an attack where a spammer can make connections to k peers in the system and publishes k.(x-1)  messages by exploiting all of its k connections. Another attack scenario is through botnets consisting of a large number of e.g., a million bots. The attacker rents a botnet and inserts each of them as a legitimate peer to the network and each can publish x-1 messages per month. Such security concerns are also discussed in this [issue](https://github.com/vacp2p/research/issues/44).<!-- Another downside is the possibility of false-positive, where the attacker can surround one single peer with some spam peers and make him look like a spammer and cause him to get removed by its neighbors. It also exhibits issues around privacy as well as arbitrary exclusion. -->

#### Economic-Incentive Spam protection
Is it the end of spam-protection journey!? shall we get along with the spammers!? Certainly not!
Waku Relay/RLN gives us a p2p spam-protection method which:
- is **efficient**  with no crazy computational, storage, memory, and bandwidth requirement! as such, it fits the  network of **heterogeneous** peers. 
...
**Zero-knowledge proof**: Zero-knowledge proof (ZKP) allows a *prover* to show a *verifier* that they know something, without revealing what that something is. This means you can do trust-minimized computation that is also privacy preserving. As a basic example, instead of showing your ID when going to a bar you simply give them a proof that you are over 18, without showing the doorman your id. In this write up, by ZKP we essentially mean zkSNARK which is one of the many types of ZKPs. 
...
<!--Conclusion, possible next steps and remaining challenges-->
...
As discussed in this post, Waku RLN Relay provides us a privacy-preserving economic spam protection mechanism through rate limiting nullifiers. The idea is to financially discourage peers from publishing more than  one message per epoch. In specific, exceeding this rate results in financial charge. 
- This solution enjoys privacy since 
  - Peers do not have to disclose any piece of personal information in any phase of this solution neither in the registration nor in the messaging phase. 
  - Peers can prove that they have not exceeded the messaging rate in a zero-knowledge manner and without leaving any trace to their membership accounts
- This solution fits heterogenous p2p messaging system since all of its computations are light. The efficiency of this also eliminates the exclusion issue of POW.
- The financial punishment of spammers is cryptographically guaranteed. 

**Future steps**: We are still at the PoC level, and the development is in progress. As our future steps, 
- we would like to investigate the runtime of Merkle tree maintenance. Indeed, the need to locally store Merkle tree on each peer was one of the unknowns discovered during this PoC and yet the concrete benchmarking result for it is not available. 
- Another possible improvement is to remove the membership contract and instead manage the group in a distributed manner e.g., through distributed hash tables. This is to address possible performance issues that may cause due to the interaction with the Ethereum blockchain. For example, the registration transactions are subject to delay due to the nature of block mining in the blockchain.
- We would also like to investigate our storage-efficient Merkle Tree maintenance solution to lower the storage overhead of peers.
...
# References
- Waku v2: https://github.com/vacp2p/specs/blob/master/specs/waku/v2/waku-v2.md
...
- RLN Benchmark: https://hackmd.io/tMTLMYmTR5eynw2lwK9n1w?view#Benchmarks
...
- Cost-effective way of member insertion and deletion: https://github.com/vacp2p/research/issues/56
- Storage overhead per peer: https://github.com/vacp2p/research/issues/57
- Storage-efficient Merkle Tree maintenance: https://github.com/vacp2p/research/pull/54
- Shamir Secret Sharing Scheme: https://en.wikipedia.org/wiki/Shamir%27s_Secret_Sharing


<!-- pow/whisper/peer scoring-->

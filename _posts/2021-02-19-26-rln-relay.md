...
Two pieces of information of the tree are important and useful for the peer. One is the root of the tree and the other is the membership proof (or the authentication path). The tree root is a public information whereas the membership proof is a private data (or more precisely the index of the peer in the tree). 

## Publishing

In order to publish at a given epoch, each message must carry a proof bundle that is a zero-knowledge proof signifying that the publishing peer is a  current registered member, and has not exceeded the messaging rate at the given epoch. Recall that the enforcement of the messaging rate was through secret shared version of the peer's `sk` and a ZKP that the secret shares are constructed correctly. 

The proof generation relies on the knowledge of peer's  `sk` and  the authentication path. Further inputs to the proof generation are the tree root, epoch and the content of the message. The proof generation results in the following data items which are:  
1. `shareX`
2. `shareY`
3. `nullifier`
4. `zkProof`
...
- RLN: https://hackmd.io/tMTLMYmTR5eynw2lwK9n1w?both
- RLN-Relay: https://github.com/vacp2p/vacp2p.github.io/blob/rln-relay/_posts/2021-02-19-26-rln-relay.md
- 
...
**Zero-knowledge proof**: Zero-knowledge proof (ZKP) allows a *prover* to show a *verifier* that they know something, without revealing what that something is. This means you can do the trust-minimized computation that is also privacy-preserving. As a basic example, instead of showing your ID when going to a bar you simply give them proof that you are over 18, without showing the doorman your id. In this write-up, by ZKP we essentially mean zkSNARK which is one of the many types of ZKPs. 
...
As discussed in this post, Waku RLN Relay provides us a privacy-preserving economic spam protection mechanism through rate-limiting nullifiers. The idea is to financially discourage peers from publishing more than one message per epoch. In specific, exceeding this rate results in a financial charge. 
...
- This solution fits the heterogenous p2p messaging system since all of its computations are light. 
